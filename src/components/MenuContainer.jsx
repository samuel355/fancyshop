            ...
            <div className='w-full px-3 flex items-center justify-start lg:justify-center gap-8 py-6 overflow-x-scroll scrollbar-none'>
                {
                    foodItems && categories.map((item)=>(
                        <motion.div whileTap={{scale : 0.75}} onClick={() => setFilter(item.urlParamName)} key={item.id} className={`group ${filter === item.urlParamName ? 'bg-red-600' : 'bg-bgCard'} w-24 min-w-[94px] h-28 cursor-pointer hover:bg-red-600 rounded-lg drop-shadow-xl flex flex-col gap-3 items-center justify-center duration-150 transition-all ease-in-out`}>
